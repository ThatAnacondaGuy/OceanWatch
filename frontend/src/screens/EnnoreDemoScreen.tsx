       ...
       <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex items-center justify-between shrink-0">
          ...
          <div className="flex gap-4">
             ...
             <div className="flex flex-col items-end">
                ...
                <span className="text-sm font-black text-red-600">Candidate (Score: {(data.attribution.results[0].attribution_score * 100).toFixed(0)}%)</span>
             </div>
          </div>
       </div>
          ...
          <div className="w-[300px] flex flex-col gap-4 shrink-0">
             <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4">
                ...
                <div className="flex flex-col gap-2">
                   <button onClick={() => navigate('/monitoring')} className="w-full py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors text-xs font-bold rounded border border-blue-200 flex items-center justify-center gap-1.5"><Eye className="w-3.5 h-3.5" /> View in Case Monitoring</button>
                   <button onClick={() => navigate('/vessels')} className="w-full py-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors text-xs font-bold rounded border border-emerald-200 flex items-center justify-center gap-1.5"><Navigation className="w-3.5 h-3.5" /> Analyze Candidate Vessels</button>
                   <button className="w-full py-2 bg-purple-50 text-purple-700 opacity-50 cursor-not-allowed text-xs font-bold rounded border border-purple-200 flex items-center justify-center gap-1.5"><FileText className="w-3.5 h-3.5" /> Generate Investigation Report</button>
                </div>
             </div>

             <div className="bg-white rounded-lg border border-slate-200 shadow-sm flex-1 flex flex-col min-h-0">
                <div className="p-3 border-b border-slate-100">
                   <h3 className="font-bold text-navy-900 text-sm">Top Candidate Vessels</h3>
                </div>
                      ...
                      <div key={r.id} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded mb-1">
                         <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                               ...
                               <span className={`text-[9px] font-bold ${r.evidence_strength === 'HIGH' ? 'text-red-500' : 'text-amber-500'}`}>{r.evidence_strength} EVIDENCE</span>
                            </div>
                         </div>
