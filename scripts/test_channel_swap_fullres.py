...
# Use a small 1000x1000 patch from the middle of the ROI at full resolution
MIN_LON, MAX_LON = -95.2, -95.1
MIN_LAT, MAX_LAT = 29.2, 29.3
...
p_wrong = run_inf(img_wrong)[mask]
p_correct = run_inf(img_correct)[mask]

print(f"WRONG (VV, VH)   -> Max: {p_wrong.max():.4f}, Mean: {p_wrong.mean():.4f}, Frac>0.5: {np.mean(p_wrong>0.5):.4f}")
print(f"CORRECT (VH, VV) -> Max: {p_correct.max():.4f}, Mean: {p_correct.mean():.4f}, Frac>0.5: {np.mean(p_correct>0.5):.4f}")
